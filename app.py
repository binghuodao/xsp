...
_crash_yin_scaled = False         # V10: sold 首阴清大半 fraction of ETF at first red day, remnant riding
_crash_yin_date = None            # V10: date of the 首阴清大半 sell (remnant T+4 window resets from here)
_crash_yin_pct = 0.75             # V10: crash ETF fraction sold at 首阴 (0.75 = 清大半 sell $3750 keep $1250; 0 = off, V9)
...
try:
    ...
    _crash_etf_scaled = bool(_pd.get('crash_etf_scaled', False))
    _crash_yin_scaled = bool(_pd.get('crash_yin_scaled', False))
    if _pd.get('crash_yin_date'):
        _crash_yin_date = datetime.strptime(_pd['crash_yin_date'], '%Y-%m-%d').date()
    _crash_resids = _pd.get('crash_resids', []) or []
    for _r in _crash_resids:
        if _r.get('expiry'):
            _r['expiry'] = datetime.strptime(_r['expiry'], '%Y-%m-%d').date()
        if _r.get('open'):
            _r['open'] = datetime.strptime(_r['open'], '%Y-%m-%d').date()
    if _pd.get('trend_opt_expiry'):
        _trend_opt_expiry = _pd['trend_opt_expiry']
        _trend_opt_strike = _pd.get('trend_opt_strike')
        _trend_opt_strike2 = _pd.get('trend_opt_strike2')
...
def send_market_report(report_type, force=False):
    ...
    global _crash_green_streak
    global _crash_yin_scaled, _crash_yin_date, _crash_yin_pct
    global _crash_resids
    global _crash_opt_reopened, _crash_opt_reopen_date
    ...
    if not direction or trend_entry_blocked or _trend_blocked:
        ...
        # Position size — $5k SPXL 3x per trade
        etf3 = 'SPXL'
        tool_recommend = {
            'etf': etf3,
        }
    ...
    if not direction:
        ...
        if single_label_s:
            ...
            _latest_report['single_mid'] = single_mid_s

        # ── 趋势滚动CALL价差 14DTE w15 ──
        if direction and is_trend:
            ...
            if (_trend_opt_expiry is None and ds_trend and dlow <= 80
                ...
                k2 = k1 + 15
                sym1 = f"US.XSP{ds_trend}C{int(k1 * 1000)}"
                sym2 = f"US.XSP{ds_trend}C{int(k2 * 1000)}"
                m1 = _opt_mid(sym1)
                m2 = _opt_mid(sym2)
                if m1 is not None and m2 is not None:
                    _trend_opt_expiry = expiry_trend
                    _trend_opt_strike = k1
                    _trend_opt_strike2 = k2
                    _trend_opt_entry = m1 - m2
                    _trend_opt_entry_date = datetime.now(ET_TZ).date()
                    _trend_opt_sigma = hs.get('vix', 20) / 100.0
                    _trend_opt_pnl = 0.0
                    ws = {"date": ds_trend, "short": str(int(k1)), "mid": "", "long": str(int(k2)),
                ...
                if dte <= 2 and ds_trend:
                    ...
                    if m1 is not None and m2 is not None:
                        # 滚仓: 关闭旧价差, 开新价差
                        if _trend_opt_entry is not None and _trend_opt_pnl is not None and _trend_opt_strike and _trend_opt_strike2:
                            old_s1 = f"US.XSP{_trend_opt_expiry[2:4]+_trend_opt_expiry[5:7]+_trend_opt_expiry[8:10]}C{int(_trend_opt_strike * 1000)}"
                            old_s2 = f"US.XSP{_trend_opt_expiry[2:4]+_trend_opt_expiry[5:7]+_trend_opt_expiry[8:10]}C{int(_trend_opt_strike2 * 1000)}"
                            o1 = _opt_mid(old_s1)
                            o2 = _opt_mid(old_s2)
                            if o1 is not None and o2 is not None:
                                _trend_opt_pnl += max(o1 - o2 - _trend_opt_entry, -_trend_opt_entry) * 100
                        _trend_opt_expiry = expiry_trend
                        _trend_opt_strike = k1
                        _trend_opt_strike2 = k2
                        _trend_opt_entry = m1 - m2
                        ...
                        if not any(g.get('date')==ws['date'] and g.get('short')==ws['short'] for g in user_watchlist):
                            ...
                            try:
                                ...
                                print(f"⚠️ Watchlist save failed: {e}")
                            socketio.emit('sync_watchlist', user_watchlist)
                        lines.append(f"🔄 CALL价差 滚仓至 {_trend_opt_expiry} {_trend_opt_strike}C / {_trend_opt_strike2}C")
                ...
                if _trend_opt_strike and _trend_opt_strike2 and _trend_opt_entry is not None:
                    sym2a = f"US.XSP{_trend_opt_expiry[2:4]+_trend_opt_expiry[5:7]+_trend_opt_expiry[8:10]}C{int(_trend_opt_strike * 1000)}"
                    sym2b = f"US.XSP{_trend_opt_expiry[2:4]+_trend_opt_expiry[5:7]+_trend_opt_expiry[8:10]}C{int(_trend_opt_strike2 * 1000)}"
                    c1 = _opt_mid(sym2a)
                    c2 = _opt_mid(sym2b)
                    if c1 is not None and c2 is not None:
                        cur_pnl = (c1 - c2 - _trend_opt_entry) * 100 + (_trend_opt_pnl or 0)
                        opt_val_line = f" | PnL ${cur_pnl:.0f}"
                lines.append(f"═══ 趋势滚动CALL价差 {_trend_opt_expiry} ═══")
                lines.append(f"{_trend_opt_strike}C / {_trend_opt_strike2}C 入场 ${_trend_opt_entry:.2f}{opt_val_line}")
                _latest_report['trend_opt_expiry'] = _trend_opt_expiry
                _latest_report['trend_opt_strike'] = _trend_opt_strike
                _latest_report['trend_opt_strike2'] = _trend_opt_strike2
    ...
    if _crash_entry_date:
        ...
        if _crash_etf_stop_pct > 0 and _crash_etf_entry:
            _crash_etf_stop = _crash_etf_entry * (1 - _crash_etf_stop_pct)
        else:
        ...
        _green_gate = 3 if _crash_exit_mode == 'V1' else _crash_force_days
        _force_ref = _crash_entry_date
        if _crash_exit_mode in ('V2', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10', 'V11') and _crash_etf_scaled and _crash_half_date:
            _force_ref = _crash_half_date
        if _crash_exit_mode in ('V4', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10', 'V11') and _crash_reentry and _crash_reentry_date:
            _force_ref = _crash_reentry_date
        if _crash_exit_mode == 'V10' and _crash_yin_scaled and _crash_yin_date:
            _force_ref = _crash_yin_date
        _force_days = len(pd.bdate_range(_force_ref, _today)) - 1 if _force_ref else crash_days

        # SPXL separate stop: exit the ETF leg only (option keeps riding) before the XSP stop can hit
        if not _crash_etf_out and _crash_etf_stop_pct > 0 and _crash_etf_entry:
            _spxl_now = _get_etf_price('SPXL')
            if _spxl_now and _spxl_now <= _crash_etf_entry * (1 - _crash_etf_stop_pct):
                _crash_etf_out = True
                lines.append(f"🛑 SPXL ETF单独止损: ${_spxl_now:.2f} ≤ ${_crash_etf_entry*(1-_crash_etf_stop_pct):.2f} (-{_crash_etf_stop_pct:.1%}), 平SPXL, 期权续持")
                close_lines.append(f"  🛑 崩盘ETF单独止损 {crash_days}d (SPXL {_crash_etf_entry:.2f}→{_spxl_now:.2f} ≤ {_crash_etf_entry*(1-_crash_etf_stop_pct):.2f}), 平SPXL, 期权续持")

        if price <= _crash_stop:
            _closed_crash_sh = _crash_keep_sh if _crash_etf_scaled or _crash_yin_scaled else _crash_shares
            if _crash_etf_scaled:
                ...
                if _crash_exit_mode in ('V9', 'V10', 'V11') and _crash_k1 and _crash_k2 and _crash_debit is not None:
                    _resid_open = _crash_entry_date or _today
                    _resid_exp = _resid_open + timedelta(days=_crash_dte)
                    ...
                    lines.append(f"🛑 崩盘跌穿止损 ${_crash_stop:.2f} (-{_crash_stop_pct:.1%}) ({_crash_entry_price:.2f}→{price:.2f}), 建议平仓")
            ...
            if _crash_exit_mode == 'V3':
                ...
                _closed_crash_sh = _crash_shares
                _crash_entry_date = None; _crash_entry_price = None; _crash_k1 = None
                _crash_k2 = None; _crash_debit = None; _crash_sigma = None
                _crash_etf_entry = None; _crash_etf_scaled = False; _crash_etf_out = False
                _crash_half_date = None; _crash_reentry = False; _crash_reentry_date = None
            ...
            if price > _crash_green:
                ...
                _crash_yin_scaled = False; _crash_yin_date = None
                _crash_green_streak = 1
            elif _force_days >= _crash_force_days:
                lines.append(f"💸 崩盘首阴续持{_crash_force_days}天: 剩SPXL {_yin_keep}股强制平仓")
                close_lines.append(f"  💸 崩盘首阴续持{_crash_force_days}天 {crash_days}d (入场${_crash_entry_price:.2f}→现价${price:.2f}), 平剩ETF {_yin_keep}股")
                _closed_crash_sh = _yin_keep
                _crash_entry_date = None; _crash_entry_price = None; _crash_k1 = None
                _crash_k2 = None; _crash_debit = None; _crash_sigma = None
                _crash_etf_entry = None; _crash_etf_scaled = False; _crash_etf_out = False
                _crash_half_date = None; _crash_reentry = False; _crash_reentry_date = None
                ...
                lines.append(f"⏳ 崩盘首阴续持: 剩SPXL {_yin_keep}股 ({_crash_force_days - _force_days}d最多 | 收复首阳${_crash_green:.2f}再进 | 止损${_crash_stop:.2f})")
        elif _crash_etf_scaled:
            _half_days = len(pd.bdate_range(_crash_half_date, _today)) - 1 if _crash_half_date else 0
            _spxl_now = _get_etf_price('SPXL')
            _is_yin = bool(_xsp_prev_close) and price < _xsp_prev_close
            _exit_kind = None
            if _is_yin:
                if _crash_exit_mode in ('V4', 'V8'):
                    _exit_kind = '首阴'
                elif _crash_exit_mode in ('V5', 'V9', 'V10', 'V11'):
                    if _spxl_now and _crash_etf_entry and _spxl_now > _crash_etf_entry:
                        _exit_kind = '首阴'
                elif _crash_exit_mode == 'V6':
                    if _half_days <= 3:
                        _exit_kind = '首阴'
                elif _crash_exit_mode == 'V7':
                    _exit_kind = '首阴' if _crash_green_streak >= 2 else '二次首阳'
            ...
            if _exit_kind == '首阴':
                ...
                _closed_crash_sh = _crash_keep_sh
                _crash_entry_date = None; _crash_entry_price = None; _crash_k1 = None
                _crash_k2 = None; _crash_debit = None; _crash_sigma = None
                _crash_etf_entry = None; _crash_etf_scaled = False; _crash_etf_out = False
                _crash_half_date = None; _crash_reentry = False; _crash_reentry_date = None
                ...
                lines.append(f"💸 已持{_crash_force_days}天: 剩余SPXL {_crash_keep_sh}股强制平仓")
                close_lines.append(f"  💸 崩盘已持{_crash_force_days}天 (入场${_crash_entry_price:.2f}→现价${price:.2f}), 平剩余ETF {_crash_keep_sh}股")
                _closed_crash_sh = _crash_keep_sh
                _crash_entry_date = None; _crash_entry_price = None; _crash_k1 = None
                _crash_k2 = None; _crash_debit = None; _crash_sigma = None
                _crash_etf_entry = None; _crash_etf_scaled = False; _crash_etf_out = False
                ...
                if _is_yin:
                    ...
                    _crash_green_streak += 1
                if _crash_reentry:
                    lines.append(f"⏳ 剩SPXL {_crash_keep_sh}股续持(再进) ({_crash_force_days - _force_days}d最多 | 止损 ${_crash_stop:.2f} (-{_crash_stop_pct:.1%}) | 首阳 ${_crash_green:.2f})")
                else:
                    lines.append(f"⏳ 剩SPXL {_crash_keep_sh}股续持 ({_crash_force_days - _force_days}d最多 | 止损 ${_crash_stop:.2f} (-{_crash_stop_pct:.1%}) | 首阳 ${_crash_green:.2f})")
        elif _crash_exit_mode == 'V10' and not _crash_yin_scaled and crash_days >= 1 and _force_days < _crash_force_days \
            ...
            lines.append(f"💸 崩盘已持{_crash_force_days}天, 强制平仓")
            close_lines.append(f"  💸 崩盘已持{_crash_force_days}天 (入场${_crash_entry_price:.2f}→现价${price:.2f}), 建议平仓")
            _closed_crash_sh = _crash_keep_sh if _crash_etf_scaled else _crash_shares
            _crash_entry_date = None; _crash_entry_price = None; _crash_k1 = None
            _crash_k2 = None; _crash_debit = None; _crash_sigma = None
            _crash_etf_entry = None; _crash_etf_scaled = False; _crash_etf_out = False
            _crash_half_date = None; _crash_reentry = False; _crash_reentry_date = None
            ...
            lines.append(f"⏳ 崩盘等待中 ({_crash_force_days - crash_days}d最多 | 止损 ${_crash_stop:.2f} (-{_crash_stop_pct:.1%}) | 首阳 ${_crash_green:.2f})")
        _latest_report['crash_force_days'] = _force_days
        _latest_report['crash_reentry'] = _crash_reentry
        _latest_report['crash_half_date'] = str(_crash_half_date) if _crash_half_date else None
        ...
        _latest_report['crash_yin_scaled'] = _crash_yin_scaled
        _latest_report['crash_yin_date'] = str(_crash_yin_date) if _crash_yin_date else None
    ...
    # ── 延迟开仓（close-before-open）：全部平仓处理之后才开新仓，三层互斥只开一个，优先级由 _layer_priority 决定 ──
    _no_layer_open = (_crash_entry_date is None and _mr_entry_date is None
    ...
    if _crash_ok and not (_mr_ok and _layer_priority == 'mr_crash_trend'):
        ...
        _crash_opt_reopened = False
        _crash_opt_reopen_date = None
        _crash_yin_scaled = False
        ...
        if not any(g.get('date')==ws['date'] and g.get('short')==ws['short'] for g in user_watchlist):
            ...
            socketio.emit('sync_watchlist', user_watchlist)
        _crash_shares = max(round(_crash_etf_size / _crash_etf_entry), 1) if _crash_etf_entry else 0
        if _closed_crash_sh is not None:
            _diff = _crash_shares - _closed_crash_sh
            if _diff == 0:
                lines.append(f"⚠️ 同价续持: 当日平仓价=新仓入场价 ${_crash_etf_entry:.2f}, SPXL {_closed_crash_sh}股 无需平仓重开, 仅换期权 {_crash_k1}C/{_crash_k2}C (省两笔手续费)")
            elif _diff > 0:
                lines.append(f"⚠️ 同价续持: 当日收盘先平后开同价, 旧 {_closed_crash_sh}股 免卖, 直接加开 {_diff}股 凑 {_crash_shares}股 (省一轮卖+买手续费)")
            else:
                lines.append(f"⚠️ 同价续持: 当日收盘先平后开同价, 旧 {_closed_crash_sh}股 免卖, 卖出 {-_diff}股 对齐新仓 {_crash_shares}股")
    ...
    if _crash_resids:
        ...
        for _ri, _res in enumerate(_crash_resids, 1):
            ...
            _rT = max(_crash_dte / 365.0 - _rdays / 365.0, 1 / 365.0)
            ...
            if price > _res['entry']:
                lines.append(f"💰 残期#{_ri}收复: 平CALL价差 (入场${_res['entry']:.2f}→现价${price:.2f}, 估${_rval:.0f})")
                close_lines.append(f"  💰 崩盘残期#{_ri}收复 {_rdays}d (入场${_res['entry']:.2f}→现价${price:.2f}), 平期权")
                _latest_report['crash_resid_closed'] = _latest_report.get('crash_resid_closed', 0) + 1
            elif _r_today >= _res['expiry']:
                lines.append(f"💸 残期#{_ri}到期兜底: 期权到期锁亏 净${_res['debit']:.2f}/口")
                close_lines.append(f"  💸 崩盘残期#{_ri}到期 {_rdays}d (入场${_res['entry']:.2f}), 期权到期锁亏")
                _latest_report['crash_resid_closed'] = _latest_report.get('crash_resid_closed', 0) + 1
            else:
                _keep.append(_res)
                lines.append(f"⏳ 残期#{_ri}续持 第{_rdays}d (收复${_res['entry']:.2f} | 到期{_res['expiry'].strftime('%m-%d')})")
        _crash_resids = _keep
        _latest_report['crash_resid_count'] = len(_crash_resids)
